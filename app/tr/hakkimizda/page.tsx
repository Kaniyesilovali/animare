...
export const metadata: Metadata = {
  ...
  openGraph: {
    title: 'Hakkımızda — Lefkoşa Veteriner Hekimleri | Animare',
    description: 'Animare Veteriner Kliniği ekibimizle tanışın.',
    url: '/tr/hakkimizda',
  },
  alternates: {
...
export default async function TrHakkimizdaPage() {
  ...
  const breadcrumb = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: about.badge, item: 'https://animare.vet/tr/hakkimizda' },
    ],
  }
  ...
  const team = {
    ...
    member: [
      { '@type': 'Person', name: about.members.vet1.name, jobTitle: about.members.vet1.role, worksFor: { '@id': 'https://animare.vet/#organization' } },
      { '@type': 'Person', name: about.members.vet2.name, jobTitle: about.members.vet2.role, worksFor: { '@id': 'https://animare.vet/#organization' } },
    ],
  }
      ...
      <section className="py-16 bg-[var(--color-surface)]">
        <div className="mx-auto max-w-6xl px-4 sm:px-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            {values.map(({ key, icon }) => (
              <div key={key} className="rounded-2xl border border-[var(--color-border)] bg-white p-6 text-center">
                <div className="text-4xl mb-3">{icon}</div>
                <h3 className="font-semibold text-[var(--color-text)] mb-2">{about.values[key].title}</h3>
